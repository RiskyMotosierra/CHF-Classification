...
To model the probability of congestive heart failure (CHF), two classification approaches were implemented: stepwise logistic regression and LASSO logistic regression. For the stepwise model, predictors were first screened using odds ratios, followed by forward and backward selection based on Akaike Information Criterion (AIC) to obtain a parsimonious subset of variables.

LASSO logistic regression was used as an alternative approach, applying an $L_1$ penalty to shrink coefficients toward zero and perform automatic variable selection. The optimal penalty parameter ($\lambda$) was selected using 10-fold cross-validation.

Model performance was evaluated using cross-validation and summarized using accuracy, precision, recall (sensitivity), and the Area Under the Receiver Operating Characteristic Curve (AUC).
\vspace{-1em}
\section*{Results}
\vspace{-6mm}
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, arc=6pt, width=\textwidth]
	\includegraphics[width=0.73\textwidth,height=0.58\textheight,keepaspectratio]{Fig_1.jpeg}
	\centering
...
Following model fitting, both stepwise logistic regression and LASSO logistic regression were evaluated using cross-validation. Performance metrics are summarized in Table 2. The LASSO model achieved higher AUC (0.8716) compared to the stepwise model (0.8207), along with slightly higher accuracy, precision, and recall. The ROC curves (Fig. 2) reflect this difference, with the LASSO model yielding consistently higher true positive rates across thresholds.
\begin{center}
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, arc=6pt, width=\textwidth]
\centering
...
\midrule
Stepwise & 0.9574 & 0.5302 & 0.1241 & 0.8207 \\
LASSO & 0.9592 & 0.6047 & 0.1318 & 0.8716 \\
\bottomrule
\end{tabular}
\captionof{table}{Cross-validated predictive performance of stepwise logistic regression and LASSO models for CHF classification, including accuracy, precision, recall, and AUC.}
\end{center}

\begin{center}
\includegraphics[width=0.8\textwidth]{rocgraph.png}
\captionof{figure}{ROC curves comparing stepwise and LASSO logistic regression models.}
\end{center}
...
This study evaluated the predictive ability of a wide range of biological, psychological, and social factors for congestive heart failure (CHF). Although both models achieved high overall accuracy, recall remained low, indicating difficulty in identifying true CHF cases. This reflects the class imbalance in the dataset, where relatively few participants reported CHF, and suggests that accuracy alone may overstate model performance.

The LASSO model demonstrated consistently better performance than the stepwise model, with higher AUC, precision, and recall. This suggests that regularization methods may be more effective in high-dimensional settings, where many predictors are weakly informative or correlated. In contrast, stepwise selection, while more interpretable, may be more sensitive to noise and model instability.

Several limitations should be considered. First, the cross-sectional design of NHANES prevents causal inference. Second, class imbalance likely limited sensitivity, and alternative approaches such as resampling or threshold adjustment may improve detection of CHF cases. Finally, while LASSO improved predictive performance, it reduced interpretability relative to traditional regression approaches.
